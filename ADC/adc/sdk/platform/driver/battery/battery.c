...
uint8_t batt_cal_beiliang_1380mA(uint16_t adc_sample)
{
    uint8_t batt_lvl;

//    //1705=3.0V, 1137=2V
//    if(adc_sample >= 611)
//        batt_lvl = (((adc_sample-611.000)/(747.000 - 611.000))*100); 
//    else
//        batt_lvl = 0;
		
		batt_lvl=(float)adc_sample*1.2/1023/82*382*10;
 ...
 ****************************************************************************************
 */
void adc_usDelay2(uint32_t nof_us)
{
    while( nof_us-- ){
        __nop();
        __nop();
...
uint8_t battery_get_lvl(uint8_t batt_type)
{
	...
	if (batt_type == BATT_AAA)
	{
		...
		if(batt_type == BEILIANG_1380mA)
		{
//			adc_sample = adc_get_vbat_sample(false);   //获取ADC 的值
			adc_init(GP_ADC_SE, GP_ADC_SIGN, 0); //0衰减输入
	    adc_usDelay2(20);
			adc_enable_channel(ADC_CHANNEL_P01);
      adc_sample = adc_get_sample();
			
//			printf_string("\rCurrent battery ADC_sample: ");           
//  	  printf_byte_dec(adc_sample);
//      printf_string("  DD\t");
    ...
    adc_sample = battery_filter_value(adc_sample);  //获取的ADC值进行滤波

		printf_string("\rCurrent battery filt_ADC_sample: ");           
  	printf_byte_dec(adc_sample);
    printf_string("\t");
		
		printf_string("\rCurrent battery filt_ADC_sample: ");           
  	printf_byte_dec((int)(((float)(adc_sample*1.2/1023))*100));
    printf_string("v\t");
		
		batt_lvl=(float)adc_sample*1.2/1023/82*382*10;
		
		printf_string("\rCurrent battery : ");           
  	printf_byte_dec((int)batt_lvl*100/42);
    printf_string("%\t");
//    switch (batt_type)
//    {
...
//            break;
//				case BEILIANG_1380mA:
//					  batt_lvl = batt_cal_beiliang_1380mA(adc_sample);
//            break;
//#else
