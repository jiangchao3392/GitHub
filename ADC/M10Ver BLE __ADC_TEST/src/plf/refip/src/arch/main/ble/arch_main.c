...
extern uint8_t uBatRemainPercent,BatBuf[10],uBatRemainPercentMA ,adbattery_status ,BatCnt;
 uint8_t  uart_COUNT;
bool uart_COUNT_flag;
uint16_t adc_data2,  uBatRemainPercent2,   uBatRemainPercentMA2  ;																	
uint8_t 	 BatBuf2[10],BatCnt2;																	
																		/*
...
//				uart_send_byte(0X00);uart_send_byte(0X00);uart_send_byte(0X00);
			}
...
//1£¬ Èí¼þ¹Ø»úÇé¿ö £¨ºÏ¸Ç£¬Ð¡ÓÚ7µÎ£¬30Ãëºó¹Ø»ú£©
			...
			if(count_present<7  && cIptWarning==0x00 && clamp!=1 )	   	
				{								
					if(SystemCount>=60&&SystemCount<=80)   //30sÄÚ¼ì²â²»µ½ÒºµÎ£¬¹Ø»úÇ°·¢ËÍ¹Ø»úÔ¤¾¯ÐÅºÅ0XCC  60*0.5=30s   80*0.5=40s
					  {  
...
/////////
// µç»úËøËÀ					
//                    printf_string("\n SUO SI app_timer_set(MOTOR_TIMER) ");			//´òÓ¡ ËøËÀÊä³ö						
...
//					printf_string("\n Detects a drop DropINT==1 ");
					...
					avg_value = (maBuffer[1]+maBuffer[2]+maBuffer[3]+maBuffer[4]+maBuffer[5]-maBuffer[6])/4;   //ÊµÏÖÒÆÎ»ÇóÆ½¾ù
...
////			}
		    ...
		    uart_COUNT++;
				...
				if(uart_COUNT==50 )   // ÖÜÆÚ£º 0.5s*20=10s    ÊÍ·Å±£³Ö£¨20-15£©*0.5s=2.5s
				{																											
						uart_COUNT=0;
						uart_COUNT_flag = true ;									

				}
//				else 
//					if(iSecCount%40==20&& Motor==1 ) // Õý·´×ª¼ä¸ô20*0.5s=10s   ±£³Ö15ºó½øÈëÊÍ·Å³ÌÐò£¬ÊÍ·Å±£³Ö20-15ºóµ½1ÔÚ½øÈëÑ¹Ëõ³ÌÐò
					
				if(uart_COUNT_flag==true )
				{
					 uart_COUNT_flag =false ;
						
//					  adc_enable_channel(2);
						adc_enable_channel(ADC_CHANNEL_P01);	
					  adc_data2 = adc_get_sample();
		
            printf_string("\rCurrent battery  ADC_value: "); 
						printf_byte_dec(adc_data2);
//					  uart_send_byte(adc_data2);
					
					
						uBatRemainPercent2 = (char)(((adc_data2-611.000)/(747.000 - 611.000))*100);   	                                                                                  			
						
					  printf_string("\rCurrent battery  uBatRemainPercent2: "); 
						printf_byte_dec(uBatRemainPercent2);
					  printf_string("% ");
						if((adc_data2-611.000)<=0 && adc_data2!=0)
						{
							uBatRemainPercent2 = 0;		
						}			
						
						BatBuf2[BatCnt2] =  uBatRemainPercent2;
						BatCnt2++;	
						
						uBatRemainPercentMA2 = fliter(BatBuf2,BatCnt2); //µç³ØµçÁ¿¾ùÖµÂË²¨Î´Íê³ÉÊ±·¢ËÍÊµÊ±Êý¾Ý
						...
						printf_string("\rCurrent battery  ADC_percentage: "); 
						printf_byte_dec(uBatRemainPercentMA2);
						printf_string("% ");
...
//			  adc_enable_channel(2);      
////        ADCCC= adc_get_sample(); 
////	      uart_send_byte(0X88);  
////	     uart_send_byte(ADCCC);	
//				printf_string("\rCurrent battery  ADC_value: "); 
//				printf_byte_dec(ADCCC);
