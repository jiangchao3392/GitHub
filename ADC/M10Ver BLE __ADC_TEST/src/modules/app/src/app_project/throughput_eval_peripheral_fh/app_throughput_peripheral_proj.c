      ...
      if(adc_data!=0 && MotorFlag!=1&&ZigbFlg!=1)
	    ...
	    if( adbattery_status==0)	   
			   {	 
					 for(i=BatCnt;i<10;i++)
					 { BatBuf[i] = 0x40; }         //64
					 uBatRemainPercentMA = fliter(BatBuf,10); //µÁ≥ÿµÁ¡øæ˘÷µ¬À≤®Œ¥ÕÍ≥… ±∑¢ÀÕ µ ± ˝æ›
			   }
			else 	 
...
{ 
				...
				uart_send_byte(0X55); 
				uart_send_byte(VIsense[VIsenseCount]);
			    VIsenseCount++;
				VIsense_Avg= (VIsense[0]+VIsense[1]+VIsense[2])/3;    //3¥Œ≤…—˘µƒ∆Ωæ˘÷µ
				if(VIsenseCount == 3)					     					   						
				{VIsenseCount = 0;}		         
						
//       printf_string("motor_forwad VIsense_Avg=");
//       uart_send_byte(VIsense_Avg );
//¿œµÁª˙∞Ê±æ		£¨—πÀı
...
//		 printf_string("motor_reset VIsense_Avg=");
//     uart_send_byte(VIsense_Avg );
...
//					app_timer_set(MOTOR_RESET, TASK_APP, 50);
					app_timer_set(MOTOR_RESET, TASK_APP, 100);    //  ≤‚ ‘
					MotorFlag = 1;
        }		
   return 0;
